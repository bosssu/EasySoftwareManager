...
''')
    ...
    try :
        for file in fileList:
            ...
            tool.Interpreter()
            csfile.append("\tpublic static Dictionary<uint, "+filename+"> "+filename+"Dict = new Dictionary<uint, "+filename+">();\n")
    except BaseException, e :
        print "Interpreter Failed!!!"
        print e
        sys.exit(-3)

    pb_file = open("TableProto.proto", "w+")
    pb_file.writelines(pblist)
    pb_file.close()
	
    try :
        command = "protoc TableProto.proto --descriptor_set_out=TableProto.protodesc"
        os.system(command)
    except BaseException, e :
        print "protoc failed!"
        raise
    ...
    csfile.append("\tpublic static void Load() {\n")
    ...
    try :
        for file in fileList:
            ...
            parser.Parse()
            csfile.append("\t\tpath = FileTools.CombinePath (TABLE_PATH, \"" +filename.lower()+ "\");\n")
            csfile.append("\t\tresource = ResourceManager.Instance.GetResource(path, typeof(TextAsset), enResourceType.TableProto);\n")
            csfile.append("\t\tbinaryObject = resource.m_content as BinaryObject;\n")
            csfile.append("\t\tstream = new MemoryStream (binaryObject.m_data);\n")
            csfile.append("\t\t"+filename+"_ARRAY "+filename+"_items = Serializer.Deserialize<"+filename+"_ARRAY>(stream);\n")
            csfile.append("\t\tfor (int i = 0; i < "+filename+"_items.items.Count; i++) {\n")
            csfile.append("\t\t\t"+filename+"Dict.Add ("+filename+"_items.items [i].id, "+filename+"_items.items [i]);\n")
            csfile.append("\t\t}\n")
    except BaseException, e :
        print "Parse Failed!!!"
        print e
    ...
    try :
        command = "ProtoGen\protogen -i:TableProto.protodesc -o:AllTableProto.cs"
        os.system(command)
    except BaseException, e :
        print "protoc failed!"
        raise
    ...
    csfile.append("\t}\n")
    csfile.append("}")
    cs_file = open("TableProtoLoader.cs", "w+")
    cs_file.writelines(csfile)
    cs_file.close()
